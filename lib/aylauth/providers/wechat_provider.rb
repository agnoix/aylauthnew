module Aylauth
  module Provider
    module WechatProvider
      extend self

      def get_auth_url
        body = { user: { application: { app_id: Aylauth::Settings.application_id,
                                        app_secret: Aylauth::Settings.application_secret },
                                        auth_method: "wechat_provider" } }
        options = { body: ActiveSupport::JSON.encode(body),
                    method: :post,
                    process_json: false}


        return Aylauth::ExternalService.call_external_service(Aylauth::Settings.user_service_user_url + "/sign_in.json", options)
      end
